document.addEventListener('DOMContentLoaded', async () => {
    ...
    function reproducirAudio(blob) {
        return new Promise((resolve, reject) => {
            if (isPlayingSequence && audioPlayer && !audioPlayer.paused) audioPlayer.pause();
    ...
    function agregarAPipa(pictograma) {
        ...
        actualizarSugerenciasPredictivas(); // Dispara el motor
    }
    ...
    if (document.getElementById('nucleo-grid')) {
        await cargarDatosGlobales(); // Precargar datos
        cargarNucleo();
        cargarCategoriasPerifericas();
        ...
        // Borrar último (Nuevo botón accesible)
        document.getElementById('borrar-ultimo-btn')?.addEventListener('click', () => {
            if (fraseActual.length > 0) {
                fraseActual.pop();
                renderizarTiraFrase();
                actualizarSugerenciasPredictivas();
        ...
        document.getElementById('back-button')?.addEventListener('click', () => {
            actualizarSugerenciasPredictivas(); // Retorna al estado inteligente anterior
        });

        // Compartir (Mantenemos solo la versión de texto como solicitaste)
